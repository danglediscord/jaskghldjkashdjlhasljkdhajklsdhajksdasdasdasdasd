...
.setColor(0x36393F)
.setDescription(`:red_square: Kayıt olunca otomatik alınacak rol sıfırlandı!`)
...
.setColor(0x36393F)
.setDescription(`:red_square: Kayıt olunca alınacak rolü belirtiniz!`)
.setThumbnail(client.user.avatarURL())
.setFooter(`Pearlys`)
...
.setTitle(`${client.user.username} | Alınacak rolü ayarlama komutu.`)
.setColor(0x36393F)
.setDescription(`:green_square: Kayıt olunca otomatik alınacak rol başarıyla ${rol} olarak ayarlandı!`)
.setThumbnail(client.user.avatarURL)
.setFooter(`Pearlys`)
