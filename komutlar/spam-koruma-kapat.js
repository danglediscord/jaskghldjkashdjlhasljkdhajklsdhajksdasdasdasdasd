...
if(!sistem) return message.channel.send(nn.setDescription(`:red_square: Spam koruma zaten aktif değil.`)).then(a => a.delete({timeout: 10000}));

data.delete(`spam.${message.guild.id}`);
return message.channel.send(nn.setTitle(`İşlem başarılı!`).setColor(0x36393F).setDescription(`:red_square: Spam koruma başarıyla kapatıldı.`)).then(a => a.delete({timeout: 10000}));
