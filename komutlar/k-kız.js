...
if (!kızrol) return message.channel.send(`:red_square: Sunucuda kız rolü ayarlanmadığı için komut kullanılamaz!`)
let member = message.mentions.members.first();
if (!member) return message.channel.send(`:red_square: Kız olarak kayıt edeceğin kullanıcıyı belirtmelisin!`)
let isim = args[1]
...
.setColor(0x36393F)
.setDescription(`:green_square: Kız olarak kayıt edilen kullanıcı: ${member} \n Kız olarak kayıt eden yetkili: <@!${message.author.id}>`)
