...
.setColor(0x36393F)
.setDescription(`:red_square: Sunucu için ayarladığınız erkek rolü başarıyla sıfırlandı!`)
...
.setColor(0x36393F)
.setDescription(`:red_square: Ayarlayacağınız erkek rolünü belirtiniz!`)
.setThumbnail(client.user.avatarURL)
.setFooter(`Pearlys`)
...
.setTitle(`${client.user.username} | Erkek rol ayarlama komutu.`)
.setColor(0x36393F)
.setDescription(`:green_square: Erkek rolü başarıyla ${rol} olarak ayarlandı!`)
.setThumbnail(client.user.avatarURL)
.setFooter(`Pearlys`)
