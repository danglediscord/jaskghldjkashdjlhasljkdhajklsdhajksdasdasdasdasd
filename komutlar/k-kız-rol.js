...
.setColor(0x36393F)
.setDescription(`:red_square: Sunucu için ayarladığınız kız rolü başarıyla sıfırlandı!`)
...
.setColor(0x36393F)
.setDescription(`:red_square: Ayarlayacağınız kız rolünü belirtiniz!`)
.setThumbnail(client.user.avatarURL)
.setFooter(`Pearlys`)
...
.setTitle(`${client.user.username} | Kız rol ayarlama komutu.`)
.setColor(0x36393F)
.setDescription(`:green_square: Kız rolü başarıyla ${rol} olarak ayarlandı!`)
.setThumbnail(client.user.avatarURL)
.setFooter(`Pearlys`)
