...
let westrabotlistyetkilisi = db.fetch(`westrabotlistyetkilirol_${message.guild.id}`)
  ...
  let sebep9 = args[11]
   ...
   if (!sebep) return message.channel.send(`:red_square: Botun onaylanmama sebebini yazmalısın.`).then(x => x.delete({timeout: 3000}))
  message.delete()
   const westrabumbeyyyy = new Discord.MessageEmbed()
    .setColor(`RED`)
    .setFooter(`Pearlys`)
    .setTimestamp()
    .setDescription(`:red_square: <@${sahip}> adlı kişinin <@${botisim}> adlı botu reddedildi. Sebep: ${sebep} ${sebep1} ${sebep2} ${sebep3} ${sebep4} ${sebep5} Reddeden yetkili: ${message.author}`)
		client.channels.cache.get(log).send(westrabumbeyyyy);// 
		message.channel.send(`:red_square: Botu reddettiniz.`).then(x => x.delete({timeout: 3000}))
};
